...
#ifndef WITH_ACTIVE_VENTILATION
    ...
    while (true)
    {
        ...
        case IDLE:
            ...
            else if (CO2_value > CO2_setpoint + VENTILATION_THRESHOLD)
            {
                ...
                Debug::println("CO2 level %.2f >> setpoint %.2f ppm. Too high", CO2_value, CO2_setpoint);
...
#else
// this version will do the ventilation when
// the gap between setpoint and current value is too large (a parameter can be set, by default 200)
// so it will actively ventilate
void CO2Controller::controlTask(void* pvParameters)
{
    auto* self = static_cast<CO2Controller*>(pvParameters);
    auto& sensor = self->sensor;
